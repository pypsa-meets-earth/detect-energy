...
"""CLI interface for iclr_osm project.

This module provides a CLI interface for the iclr_osm project.
...
from iclr_osm.eo import get_osm_data
from iclr_osm.gfk_data import get_all_valid_list, view_regions


def main():  # pragma: no cover
    """
    The main function executes on commands:
    `python -m iclr_osm` and `$ iclr_osm `.
    ...
    parser = argparse.ArgumentParser(
        description='iclr-osm by redacted-org',
        # epilog='''Example:''',
    ...
    if args.command == 'view':
        ...
        if args.regions:
            ...
            if not set(region_list) <= set(get_all_valid_list()):
                raise KeyError(f'Invalid Region {" ".join(set(region_list) - set(get_all_valid_list()))} , run iclr_osm view regions to view valid regions')
        # elif args.coords:
        ...
        if args.features:
            ...
            if not set(feature_list) <= set(primary_feature_element[args.primary]):
                raise KeyError(f'Invalid Feature {" ".join(set(feature_list) - set(primary_feature_element[args.primary]))}, run iclr_osm view features to view valid features')
        else:
        ...
        # import inquirer #https://redacted.web/magmax/python-inquirer
